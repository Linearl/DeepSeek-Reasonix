...
func appendCorePolicies(prompt string) string {
	for _, policy := range []string{config.UserDecisionPolicy, config.WorkPracticePolicy, config.AutonomyPolicy, config.LanguagePolicy, config.ContextManagementPolicy} {
		prompt += "\n\n" + policy
	}
	return prompt
}
