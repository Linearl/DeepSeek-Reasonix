...
func TestToolStartedPreservesPersistedKindValues(t *testing.T) {
	legacy := []struct {
		...
		// Fork offset: CompactionProgress (streamed chunked-compaction progress,
		// 88f40a2e2) sits between SessionChanged and ReadStatus, so every Kind from
		// ReadStatus on is one higher than upstream. The fork-only kind is appended after
		// the legacy range precisely so the values before it stay wire-stable.
		{"ReadStatus", ReadStatus, 32},
	}
