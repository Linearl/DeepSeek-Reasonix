...
// recovery gate must not arm, even when the interactive posture is auto.
func TestAutopilotDisablesAutoGuardArming(t *testing.T) {
	...
	c.mu.Unlock()
	if gate == nil {
		t.Skip("controller constructed without a recovery gate")
	}
	st := gate.Snapshot()
	for _, task := range st.Tasks {
		if task != nil && task.Failure != nil {
			t.Fatalf("autopilot must not arm Auto Guard failure: %+v", task.Failure)
		}
	}
}
...
// TestAgentSkipToolRecoveryFenceSetter is a compile-time smoke for task 107.
func TestAgentSkipToolRecoveryFenceSetter(t *testing.T) {
	reg := tool.NewRegistry()
	ag := agent.New(&noopProv{}, reg, agent.NewSession("sys"), agent.Options{}, event.Discard)
	ag.SetSkipToolRecoveryFence(true)
	ag.SetSkipToolRecoveryFence(false)
}
