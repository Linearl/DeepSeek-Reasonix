...
type Settings interface {
	...
	// subsequent turns (#9004, fork desktop integration). It persists to the
	// session's BranchMeta so the choice survives restart; it is otherwise
	// in-memory (no controller rebuild, no prompt-cache impact).
