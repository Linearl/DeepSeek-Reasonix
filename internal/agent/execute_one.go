...
func (a *Agent) applyDeliveryPolicyGates(turn *turnRuntime, plan *toolCallPlan) (toolOutcome, bool) {
	...
	if plan.evidenceName == "bash" {
		...
		// already exposes each segment's status (&&/;/| semantics are
		// transparent to the model), and a combined step avoids the extra agent
		// round trips that a split would cost with no real safety gain. Only
		// closed-loop turns keep the strict barrier, because there a mutation
		// invalidates the verification receipt even when the exit status is
		// honest.
