...
func (a *Agent) compactToProjectionLocked(ctx context.Context, trigger, instructions string, force, mustFree bool) (CompactionOutcome, error) {
	...
	// Cap every non-manual summary input (#9572), including pressure folds
	// after projection invalidation. mustFree (a true physical overflow, est
	// at or above the hard ceiling — including the over-ceiling manual rescue
	// merged in #9474) also attempts the cap, but when no balanced prefix
	// exists it falls through to the uncapped fold instead of hard-failing.
