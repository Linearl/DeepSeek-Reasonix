...
func (m ContextManager) foldContext(ctx context.Context, prepared PreparedContext, policy ContextPreparePolicy, inputHash string, est, fold, hard int, forceFold bool) (PreparedContext, error) {
	...
	for range maxSummaries {
		...
		// view cannot pay for another summary (#9572 follow-up). The
		// over-ceiling manual rescue merged in #9474 lands here too: est at or
		// above the hard ceiling is mustFree regardless of trigger.
		mustFree := result.InputTokens >= hard
