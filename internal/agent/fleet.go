...
func (*FleetTool) Description() string {
	return "Dispatch 2–64 sub-agent tasks as a small dependency graph and return bounded previews plus stable Subagent references for full-result retrieval from completed persisted children with read_subagent_result. Each item may select a profile, model, effort, tools, write_paths, worktree_root, or read_only, and may declare depends_on to run after other items (research → implement → review). Tasks with no dependency between them run in parallel and must declare non-overlapping write_paths; ordered tasks may share paths. When the concrete targets are known, prefer declaring individual file paths over whole directories, since declaring a directory that only contains other writers' disjoint files needlessly serializes those parallel writers. Omitted write_paths claim the whole workspace, so two or more concurrent writers without paths fail preflight before any task starts. Concurrent items that declare worktree_root in the same repository are also preflight-checked for overlapping changed files (merge conflicts) before anything starts. A failed task's dependents are skipped; independent branches keep going unless fail_fast is set. Background mode returns a fleet job id collectable with wait."
}
