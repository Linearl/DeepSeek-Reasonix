...
type Message struct {
	...
	// PresentedFiles is trusted, versioned host metadata for explicit file deliverables.
	// It carries no bytes or auth tokens; hosts revalidate on access.
	PresentedFiles *PresentedFilesMetadata `json:"presented_files,omitempty"`
}

// PresentedFile identifies a user-facing file deliverable without file bytes.
...
// PresentedFilesMetadata is the durable, host-only envelope for file deliverables.
