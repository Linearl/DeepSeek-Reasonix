...
| **合并恢复副本** | 会话右键 → **合并恢复副本**（或项目树中的「会话版本」入口）：查看**版本链**（各版本轮数 / 独有内容 / 可加备注 / 内容预览），**由用户自己选定「设为默认版本」** —— 主线明显偏短时系统只**提示**「主要内容可能在分支里」，不会自动替你选；合并时把落选链中主链缺失的前缀段并入，**被完全覆盖的副本归档进可恢复的回收区**；**若副本未覆盖当前对话记录则拒绝合并**（`recovery.consolidateBlocked`），不会静默丢内容。 |
