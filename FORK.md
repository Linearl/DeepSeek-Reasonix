...
| **合并恢复副本** | **设置 → 存储与路径 → 工作阶段版本与副本 →「查看并合并会话副本…」**。列出所有带 recovery 副本的工作阶段，每行给 **主线 / 总计 / 共有 / 独有 / 孤立（主线已缺失）** 五项计数 —— **「独有」= 仅存在于该副本、未并入主线的事件数，数值大就说明那边有会丢的工作**；可先「**预览分支**」并「**查看尾部消息（确认现场）**」，再勾选「**合并所选**」（另有全量扫描 / 刷新）。合并把落选链中主链缺失的前缀段 graft 进新主线；**被完全覆盖的副本**归档进可恢复的回收区；**副本未覆盖当前记录时拒绝合并**。 |
...
8. **双方案并存：上游方案作保底，我们的更优方案作实验特性**（2026-09-17 用户确立）
   - 遇到「上游已有一个实现，而我们认为还有更好做法」时，**不要二选一，两个都要**：
     - **保底路径 = 上游已探索过的方案**（默认行为）。它经过上游验证、零额外维护成本，且**追齐上游时天然对齐**。
     - **增强路径 = 我们的更优方案**，挂在 `experimental_*` 开关后（**默认关**），开关打开时才替换/叠加保底行为。
   - **关闭开关时，UI 与行为应与上游一致**；打开开关才体现 fork 独有交互。
   - **落地要求**：
     - 增强路径须在 `scripts/check-fork-integrity.mjs` **登记锚点**（防被 merge 静默丢弃，同第 6 条）。
     - 保底路径必须**保持可用**——不得因为加了增强就删掉上游入口（按钮 / 菜单项 / 快捷键等）。
     - 任务单里要**同时写明两条路径的行为与验收**，不能只写增强路径。
   - **为什么值得这么做**：这条同时消解了「fork 魔改 vs 追齐上游」的长期矛盾——**默认路径即上游路径**，创新被隔离在开关之后，merge 时不必再为独有交互反复打补丁或手工恢复。
   - **实例（任务 160，2026-09-17）**：加载更早消息。上游已把该交互改为**按钮驱动**（`components/Transcript.tsx` 的 `chat-older` 按钮），fork 原有滚动触发因 `towardHistory` 要求 `scrollTop` 严格变小而在「已在顶部」时失效。**决策：默认补上上游的按钮（保底路径）**，**滚动触发改为 `experimental_auto_load_older` 开关控制（增强路径，默认关）**。
...
- **包版本带构建时间戳**（2026-09-16 起）：本分支会长期停在 `1.38.3`，多个包共用同一版本号无法区分，
  因此包版本采用 `1.38.3-YYYYMMDD-HHMM`，**精确到分钟**（不到秒），例如 `1.38.3-20260916-1205`。
  于是 `versions/<name>/` 目录天然唯一，可回退到指定的那一次构建。两点约束：① 格式受
  `installlayout.versionDirRE` 限制，**只接受 `-` 后缀，不接受 `+`**；② release notes 文件名
  与包版本同名（`FORK-v1.38.3-20260916-1205.md`），workflow 的精确名查找直接命中，无需额外映射。
