...
本版本聚焦五件事：**让 bash 等耗时工具执行期间用户可继续对话**（实时引导）、**让 MiMo 模型推理档位自动可用**、**扩展 serve 模式能力**（多项目浏览 + 图片上传）、**压缩后自动刷新持久记忆**、以及**全新的已授权写目录管理 + 全局公共目录**（让 DeepSeek 乱写随机子目录不再反复弹审批，详见下文 #9167）。同时修复了 14 项体验问题（标准模式混合命令误拦、shell contract 误拦、归档死循环、GFM 表格流式渲染、压缩面板不刷新等）。
