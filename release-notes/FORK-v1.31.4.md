...
**Reasonix Fork v1.31.4 — 实时引导 + MiMo 兼容 + serve 端点扩展 + 压缩刷新记忆**

本版本聚焦四件事：**让 bash 等耗时工具执行期间用户可继续对话**（实时引导）、**让 MiMo 模型推理档位自动可用**（全域 `*.xiaomimimo.com` 检测）、**扩展 serve 模式能力**（多项目浏览 + 图片上传）、以及**压缩后自动刷新持久记忆**（跨会话记忆不再脱节）。同时修复了 13 项体验问题（shell contract 误拦、归档死循环、GFM 表格流式渲染、压缩面板不刷新等）。
