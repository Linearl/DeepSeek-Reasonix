...
- **手动压缩进行中，主面板显示明确的「正在压缩对话…」横幅**：手动 `/compact` 在后台异步执行，此后端 `CompactionStarted` 到 `CompactionDone` 之间可能需要数秒到十余秒（含一次摘要 LLM 调用）。此前这段窗口内主面板没有任何整体「压缩中」警示，仅在聊天底部出现一条小 pending 卡，容易让用户误以为「面板已刷新 / 压缩已完成」，而实际还没完成（底部转圈仍在转）。现在新增会话级 `compactionActive` 状态（`CompactionStarted` 置位、`CompactionDone`（含失败/中止）清除），在转录区顶部常驻一条「正在压缩对话…」横幅，直到压缩真正结束才消失——最终状态（转成完成卡 + 刷新右侧上下文面板）依旧严格绑定 `compaction_done` 事件。
