...
### 长任务体验改进（2026-09-09，任务 35）
- **只读调研预算可延长**：只读任务连续 10 轮触发"收敛"引导时，模型可先判断是否需要深入——确需则调用新工具 `extend_research_budget`（附理由）把上限加倍 **10 → 20 → 40 → 80 轮**（每回合至多 3 次，耗时门同步按 2×/4×/8×/16× 放宽）；用尽后回到原有收敛行为。此前撞线只能收敛，长调研容易被硬截断
- **前台命令超时 120s → 300s**：`wails build`、`go test ./...` 等真实构建不再被两分钟上限打断（`bash_timeout_seconds` 可显式覆盖，0 表示不限）
- **todo 清单可整份重写**：完成项可重排、当前项可替换（并行子代理乱序完成不再卡死）；仍拒绝"凭空新增完成项""完成项消失或回退""清空清单"，且拒绝信息会给出可执行的修正方式
- **能力批量拒绝**：`use_capability` 新增 `capability_ids`，一次调用可拒绝多个无关能力（此前每轮数十个候选需逐个调用）
