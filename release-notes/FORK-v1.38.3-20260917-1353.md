# Reasonix Fork 桌面版 v1.38.3-20260917-1353 — Release Notes

> **构建**：2026-09-17 13:5x ｜ **安装目录**：`versions/v1.38.3-20260917-1353/`
...
## 本版新增：三条用户反馈修复（项目路径去重 / 前台消息即时渲染 / 消息带双方 ID）

- **项目路径重复**：`migrateLegacyWorkspacesIntoProjects` 的 seen 集合改用 `projectRootKey`（大小写折叠），禁止两个项目共用同一路径
- **前台消息即时渲染**：对**当前前台标签页**优先用 mid-turn steer 投递，消息当轮即渲染（不再需要重启）；steer 被拒时自动回落到排队 follow-up
- **消息带双方 ID**：投递文案总是输出 `From contact_id` 和 `To contact_id`（空值用 `(未登记)`/`(未知)` 占位），接收方可按 ID 确认未发错；发送方未登记时明确说明无法自动回复
