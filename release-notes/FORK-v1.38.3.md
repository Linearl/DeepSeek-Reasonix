...
>
> **本文档是 `1.38.3` 基线的完整说明**（追齐内容 + 该版本线上的全部 fork 修复与增强）。
> 单独一次构建的差异不写在这里，见同名的带时间戳文件，例如
> [`FORK-v1.38.3-20260916-1205.md`](./FORK-v1.38.3-20260916-1205.md)（该包起，包版本采用
> `1.38.3-YYYYMMDD-HHMM`）。
...
- **版本号仍为 1.38.3**（本轮不递增，与上游对齐）。其后从 `1.38.3-20260916-1205` 起，包版本改为带构建时间戳 `1.38.3-YYYYMMDD-HHMM`，以便区分同一版本号下的多次构建。

## 🔧 2026-09-16 更新

> 本批已单独出包（`1.38.3-20260916-1205`），**逐项说明见
> [`FORK-v1.38.3-20260916-1205.md`](./FORK-v1.38.3-20260916-1205.md)**。

摘要：设置 → 实验特性的**「会话存储」开关**（含此前缺失的渲染表条目修复）、**需重启设置的一键重启**、
侧栏「实验特性」图标修复；发布链方面的**时间戳版本号**、release notes 查找的静默降级修复。
