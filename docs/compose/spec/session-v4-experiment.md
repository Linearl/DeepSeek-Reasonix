...
- [x] T4: Minimal control/boot wiring via SessionV4Bridge + Service entry points BindFresh/ContinueLegacy/Open — acceptance: flag off unchanged; flag on NewSession/Clear bind fresh v4 ids; Resume imports legacy; Idle HistoryPage prefers Query; bridge tests cover three entry points (covers: S2; depends: T2,T3)
