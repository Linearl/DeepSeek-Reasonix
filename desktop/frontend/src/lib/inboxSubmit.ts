...
export async function resolveActiveTurnId(binding: Pick<AppBindings, "ListTabs">, tabId: string, known?: string): Promise<string | undefined> {
  ...
  // Fork: fall back to the caller's known turn id when the tab owner cannot
  // answer (controller rebuild / recovery fork / backend busy — #9944 family).
  // Dropping it here self-locks every later ask/steer submit.
...
export async function steerInboxItemForActiveTurn(binding: ActiveTurnBindings, tabId: string, itemId: string, knownTurnId?: string) {
  ...
  // Fork: degrade to the session-level steer instead of failing (owner may be
  // another process; the item stays durable either way).
