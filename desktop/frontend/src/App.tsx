...
import { sidebarImAccessModeLabel, sidebarImAccessStatusClass, sidebarImAccessStatusLabel, sidebarImConnectionsFromBot, sidebarImScopeLabel, sidebarImSessionLabel, sidebarImSessionTarget, sidebarImTopicSourcesFromBot, type SidebarImConnection } from "./app-runtime/sidebarImProjection";
import { useSidebarImOwner } from "./app-runtime/useSidebarImHooks";
...
export default function App() {
  ...
  if (paneSwitchSeenRef.current !== visibleTranscriptTabId) {
    paneSwitchSeenRef.current = visibleTranscriptTabId;
    paneSwitchAtRef.current = { tabId: visibleTranscriptTabId ?? "", at: performance.now() };
  }
  useLayoutEffect(() => {
    const started = paneSwitchAtRef.current;
    if (!started.tabId) return;
            ...
            <div className="topicbar__actions">
              ...
              <Tooltip label={t("questionSearch.label")}>
                <button
                  className="topicbar__action-btn topicbar__action-btn--icon topicbar__action-btn--utility"
                  type="button"
                  aria-label={t("questionSearch.label")}
                  aria-expanded={questionSearchOpen}
                  onClick={() => setQuestionSearchOpen((open) => !open)}
                >
                  <ListTree size={15} />
                </button>
              </Tooltip>
