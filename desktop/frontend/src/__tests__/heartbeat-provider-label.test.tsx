...
// while the option value keeps the internal `name` identity — that is what
// `<provider>/<model>` refs persist. Resolution order: explicit `display_name` >
// built-in default label > `name`; a name the default table does not know must still
...
// Mirrors this install's real config: connections with an explicit display name, the
// ids from the bug report that had none ("minimax-M3"), a name the default table does
// not know, an auto-generated uuid connection with and without a display name, and a
// connection without models that must stay out of the picker.
const providerFixture = [
  { name: "mimo-pro", displayName: "MiMo Pro", models: ["mimo-v2.5-pro"] },
  { name: "minimax-M3", models: ["MiniMax-M3"] },
  { name: "acme-relay", models: ["acme-1"] },
  { name: "opencode-go-3d668098626d385cb9e2084d75c5db36", displayName: "OpenCode Go (Recommended) · 3", models: ["opencode-go-default"] },
  { name: "opencode-go-anthropic-3d668098626d385cb9e2084d75c5db36", models: ["opencode-go-anthropic-default"] },
...
const defaulted = optionFor("minimax-M3");
const unknown = optionFor("acme-relay");
const autoGenerated = optionFor("opencode-go-3d668098626d385cb9e2084d75c5db36");
const autoGeneratedBare = optionFor("opencode-go-anthropic-3d668098626d385cb9e2084d75c5db36");
...
ok(defaulted?.textContent?.includes("MiniMax M3") === true, "no display name: option shows the built-in default label");
ok(defaulted?.value === "minimax-M3", "no display name: option value stays the internal provider name");
ok(unknown?.textContent?.includes("acme-relay") === true, "unknown name: option still renders the name byte-for-byte");
ok(unknown?.value === "acme-relay", "unknown name: option value stays the internal provider name");
...
ok(autoGeneratedBare?.textContent?.includes("opencode-go-anthropic (3d66)") === true, "auto-generated uuid connection without a label shows a short code");
ok(options.every((option) => option.value !== "models-less"), "connections without models stay out of the picker");
ok(options.length === 6, `placeholder plus five connections are listed (got ${options.length})`);
